//
//     Generated by private class-dump
//

@interface LVCommonPoint : NSObject {
    struct shared_ptr<CutSame::CommonPoint> { struct CommonPoint *__ptr_; struct __shared_weak_count *__cntrl_; } _cppmodel;
}

@property (nonatomic) double x;
@property (nonatomic) double y;

- (id)initWithCPPModel:(struct shared_ptr<CutSame::CommonPoint> { struct CommonPoint *x0; struct __shared_weak_count *x1; })arg0;
- (id)copyToNewObject;
- (void)copyCategoryToNewObject:(id)arg0;
- (struct shared_ptr<CutSame::CommonPoint> { struct CommonPoint *x0; struct __shared_weak_count *x1; })cppmodel;
- (void)setCPPModel:(struct shared_ptr<CutSame::CommonPoint> { struct CommonPoint *x0; struct __shared_weak_count *x1; })arg0;
- (void).cxx_destruct;
- (id)copyWithZone:(struct _NSZone { } *)arg0;
- (void)setY:(double)arg0;
- (double)y;
- (double)x;
- (void)setX:(double)arg0;
- (id).cxx_construct;

@end
