//
//     Generated by private class-dump
//

@class NSString, AWEAwemeModel, AWELivePreStreamMessageManager, IESLiveDataSyncManager, NSHashTable;

@interface AWELivePreStreamMessageActionCreator : IESLiveIMBaseActionCreator <HTSLiveMessageSubscriber, IESLiveDataSyncDelegate> {
    BOOL _isCallingStart;
    AWEAwemeModel *_model;
    AWELivePreStreamMessageManager *_dispatcherManager;
    IESLiveDataSyncManager *_wrdsManager;
    NSHashTable *_subscribersForAll;
}

@property (retain, nonatomic) AWEAwemeModel *model;
@property (retain, nonatomic) AWELivePreStreamMessageManager *dispatcherManager;
@property (retain, nonatomic) IESLiveDataSyncManager *wrdsManager;
@property (retain, nonatomic) NSHashTable *subscribersForAll;
@property (nonatomic) BOOL isCallingStart;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (id)dispatcherManager;
- (void)updateAwemeModel:(id)arg0;
- (void)setDispatcherManager:(id)arg0;
- (void)updateDispatcherManager:(id)arg0;
- (id)subscribersForAll;
- (void)setSubscribersForAll:(id)arg0;
- (void)updateLiveDataBy:(id)arg0;
- (BOOL)isCallingStart;
- (void)realStart;
- (id)setupIMDispatcherWithConfig:(id)arg0;
- (void)setWrdsManager:(id)arg0;
- (id)wrdsManager;
- (void)subscribedSyncDataUpdatedWithValue:(id)arg0 oldVersion:(long long)arg1 newVersion:(long long)arg2;
- (void)startAll;
- (void)updateNtpDiffTime:(long long)arg0;
- (id)decideReceiver;
- (void)setIsCallingStart:(BOOL)arg0;
- (void)setModel:(id)arg0;
- (void).cxx_destruct;
- (id)model;
- (id)initWithConfig:(id)arg0;
- (void)dealloc;
- (void)stopAll;
- (void)messageReceived:(id)arg0;

@end
