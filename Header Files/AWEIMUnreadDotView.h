//
//     Generated by private class-dump
//

@class UILabel;

@interface AWEIMUnreadDotView : UIView {
    BOOL _shouldDisablePlus;
    double _unreadDotHeight;
    UILabel *_dotLabel;
    long long _count;
}

@property (retain, nonatomic) UILabel *dotLabel;
@property (nonatomic) long long count;
@property (nonatomic) double unreadDotHeight;
@property (nonatomic) BOOL shouldDisablePlus;

+ (id)defaultDotColor;
+ (id)messageTabDotViewStyle;
+ (double)getUnreadDotViewHeight;
+ (double)getUnreadDotViewFixWidth;
+ (double)getUnreadDotViewFontSize;
+ (id)defaultDotTextColor;
+ (void)setDefaultDotColor:(id)arg0;
+ (void)setDefaultDotTextColor:(id)arg0;

- (void)_appear;
- (id)dotLabel;
- (void)_disappear;
- (void)setDotLabel:(id)arg0;
- (long long)countOfShowingNum;
- (double)getDotHeight;
- (BOOL)shouldDisablePlus;
- (void)updateToDotViewWithSize:(struct CGSize { double x0; double x1; })arg0;
- (void)updateWithoutDotLabelWithSize:(struct CGSize { double x0; double x1; })arg0 count:(long long)arg1;
- (void)setDotFont:(id)arg0;
- (void)setDotLabelAlpha:(double)arg0;
- (void)setDotTextColor:(id)arg0;
- (double)unreadDotHeight;
- (void)setUnreadDotHeight:(double)arg0;
- (void)setShouldDisablePlus:(BOOL)arg0;
- (void)setCount:(long long)arg0;
- (void).cxx_destruct;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })arg0;
- (void)setLabelAlignment:(long long)arg0;
- (long long)count;
- (void)setLabelHidden:(BOOL)arg0;
- (void)updateWithCount:(long long)arg0;
- (void)setDotColor:(id)arg0;
- (void)updateWithText:(id)arg0;

@end
