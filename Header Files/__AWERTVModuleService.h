//
//     Generated by private class-dump
//

@class RxTwoTuple, NSString;
@protocol RTVUserProfileManagerInterface, RxInjector, RTVXRModuleService, RTVInteractionConfigureManagerInterface, RTVSettingsManager;

@interface __AWERTVModuleService : HTSService <HTSService, AWERTVModuleService> {
    id<RxInjector> _injector;
    id<RTVXRModuleService> _rtvService;
    id<RTVSettingsManager> _settingsManager;
    id<RTVUserProfileManagerInterface> _profileManager;
    RxTwoTuple *_trackEnterConvIDAndMethod;
    id<RTVInteractionConfigureManagerInterface> _interactionConfigMananger;
}

@property (readonly, weak, nonatomic) id<RxInjector> injector;
@property (readonly, nonatomic) id<RTVXRModuleService> rtvService;
@property (readonly, nonatomic) id<RTVSettingsManager> settingsManager;
@property (retain, nonatomic) id<RTVUserProfileManagerInterface> profileManager;
@property (retain, nonatomic) RxTwoTuple *trackEnterConvIDAndMethod;
@property (readonly, nonatomic) id<RTVInteractionConfigureManagerInterface> interactionConfigMananger;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)onServiceInit;
- (void)rxAwakeFromPropertyInjection;
- (BOOL)isVoipEnable;
- (BOOL)isInRTV;
- (BOOL)isSceneEnable;
- (BOOL)isCameraRelatedFunctionEnabled;
- (BOOL)isMircophoneRelatedFunctionEnabled;
- (BOOL)isRTVAudioEnableWithUid:(id)arg0 scene:(unsigned long long)arg1 shortConversationID:(id)arg2;
- (BOOL)isRTVVideoEnableWithUid:(id)arg0 scene:(unsigned long long)arg1 shortConversationID:(id)arg2;
- (BOOL)isFeedSharing;
- (id)feedShareChatConversationID;
- (BOOL)viewIsNarrowVisible;
- (BOOL)isSwitchLayoutAnimating;
- (id)rtvFeedCastVideoService;
- (BOOL)shouldDisplayIMPushOfSender:(id)arg0 convID:(id)arg1;
- (BOOL)shouldDisplayIMPushForVoipMessageIsIn1to1Chat:(BOOL)arg0;
- (id)enterFromWithMsgConvID:(id)arg0;
- (id)enterMethodWithMsgConvID:(id)arg0;
- (void)messageListAppear:(BOOL)arg0 withConvID:(id)arg1;
- (id)fetchRemoteImageWithName:(id)arg0;
- (id)fetchRemoteThemeImageWithName:(id)arg0;
- (id)getStorage;
- (BOOL)isSupportPIP;
- (void)showInviteFriendsWithType:(long long)arg0 maxCount:(long long)arg1 completion:(id /* block */)arg2;
- (void)__handleEnterMessageList:(id)arg0;
- (void)setTrackEnterConvIDAndMethod:(id)arg0;
- (void)__forceInitService;
- (id)rtvService;
- (unsigned long long)__convertScene:(unsigned long long)arg0;
- (id)trackEnterConvIDAndMethod;
- (id)interactionConfigMananger;
- (id)_showInviteFriendsWithType:(long long)arg0 maxCount:(long long)arg1;
- (void)setProfileManager:(id)arg0;
- (void).cxx_destruct;
- (id)injector;
- (id)settingsManager;
- (id)profileManager;

@end
