//
//     Generated by private class-dump
//

@class NSString, NSTimer, DYRouterModel;

@interface DYReBindFillInNewCodeViewModel : AWEBaseViewModel {
    BOOL _enableNextButton;
    BOOL _isSecondChance;
    BOOL _showResetPasswordPage;
    NSString *_errorMessage;
    long long _curCountDown;
    NSTimer *_timer;
    DYRouterModel *_context;
}

@property (copy) NSString *errorMessage;
@property BOOL enableNextButton;
@property long long curCountDown;
@property (retain) NSTimer *timer;
@property (retain) DYRouterModel *context;
@property BOOL showResetPasswordPage;
@property BOOL isSecondChance;

- (void)setCurCountDown:(long long)arg0;
- (void)countDownAction:(id)arg0;
- (long long)curCountDown;
- (void)setCountDownAndTimer:(long long)arg0;
- (void)handleSendCode;
- (void)handleNextAction:(id)arg0;
- (void)handleCaptchaChange:(id)arg0;
- (void)setEnableNextButton:(BOOL)arg0;
- (id)uidType;
- (BOOL)enableNextButton;
- (void)trackError:(id)arg0 api:(id)arg1;
- (void)__showHumanReviewAlertDialog:(id)arg0 error:(id)arg1;
- (void)showRebindSuccessToastIsSafeMobile:(BOOL)arg0;
- (void)setShowResetPasswordPage:(BOOL)arg0;
- (void)trackBundlingResult:(id)arg0;
- (void)__trackHumanReviewAlertDialogShow;
- (BOOL)showResetPasswordPage;
- (id)timer;
- (id)initWithContext:(id)arg0;
- (id)init;
- (void).cxx_destruct;
- (void)setContext:(id)arg0;
- (void)fetchData;
- (void)setTimer:(id)arg0;
- (id)context;
- (void)dealloc;
- (id)errorMessage;
- (void)setErrorMessage:(id)arg0;
- (void)setIsSecondChance:(BOOL)arg0;
- (BOOL)isSecondChance;

@end
