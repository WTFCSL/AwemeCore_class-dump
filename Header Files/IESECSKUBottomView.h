//
//     Generated by private class-dump
//

@class UIStackView, UIView, NSArray, NSString, IESECGoodsSoldOutModel, UIImageView, IESECSKUBottomViewModel, IESECSKUBottomSoldOutView, NSObject, NSMutableArray, IESECSKUDetailContext, UILabel;
@protocol OS_dispatch_source, IESECSKUBottomViewDelegate;

@interface IESECSKUBottomView : UIView <IESECSKUBottomSoldOutViewDelegate, IESECSKUBottomViewProtocol> {
    IESECSKUBottomViewModel *_viewModel;
    IESECGoodsSoldOutModel *_soldOutModel;
    id<IESECSKUBottomViewDelegate> delegate;
    IESECSKUDetailContext *detailContext;
    UIView *_deliverBgView;
    UILabel *_deliverLabel;
    UIImageView *_deliverIconImageView;
    UILabel *_countdownLabel;
    NSObject<OS_dispatch_source> *_timer;
    IESECSKUBottomSoldOutView *_soldOutView;
    NSArray *_buttonModels;
    UIStackView *_buttonStackView;
    NSMutableArray *_buttons;
}

@property (retain, nonatomic) UIView *deliverBgView;
@property (retain, nonatomic) UILabel *deliverLabel;
@property (retain, nonatomic) UIImageView *deliverIconImageView;
@property (retain, nonatomic) UILabel *countdownLabel;
@property (retain, nonatomic) NSObject<OS_dispatch_source> *timer;
@property (retain, nonatomic) IESECSKUBottomSoldOutView *soldOutView;
@property (copy, nonatomic) NSArray *buttonModels;
@property (retain, nonatomic) UIStackView *buttonStackView;
@property (retain, nonatomic) NSMutableArray *buttons;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;
@property (retain, nonatomic) IESECSKUBottomViewModel *viewModel;
@property (retain, nonatomic) IESECGoodsSoldOutModel *soldOutModel;
@property (weak, nonatomic) id<IESECSKUBottomViewDelegate> delegate;
@property (weak, nonatomic) IESECSKUDetailContext *detailContext;

- (id)detailContext;
- (void)setDetailContext:(id)arg0;
- (void)setCountdownLabel:(id)arg0;
- (id)countdownLabel;
- (void)soldOutCountDownDidFinish;
- (BOOL)isShowSoldOut;
- (id)deliverBgView;
- (id)deliverLabel;
- (id)deliverIconImageView;
- (id)soldOutView;
- (id)soldOutModel;
- (void)disableConfirmButton;
- (void)enableConfirmButton;
- (void)updateSoldOutInfo:(id)arg0;
- (void)setSoldOutModel:(id)arg0;
- (void)setDeliverBgView:(id)arg0;
- (void)setDeliverLabel:(id)arg0;
- (void)setDeliverIconImageView:(id)arg0;
- (void)setSoldOutView:(id)arg0;
- (id)timer;
- (void).cxx_destruct;
- (void)setButtons:(id)arg0;
- (id)buttons;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })arg0;
- (void)setViewModel:(id)arg0;
- (id)delegate;
- (void)setTimer:(id)arg0;
- (id)viewModel;
- (void)setDelegate:(id)arg0;
- (void)dealloc;
- (void)setupTimer;
- (void)setupView;
- (double)viewHeight;
- (id)buttonStackView;
- (void)setButtonStackView:(id)arg0;
- (id)buttonModels;
- (void)setButtonModels:(id)arg0;
- (void)setupButtons;
- (void)reloadView;
- (void)confirm:(id)arg0;

@end
