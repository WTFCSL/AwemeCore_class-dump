//
//     Generated by private class-dump
//

@class UIStackView, NSArray, IESECGoodsFeedPopoverItemContentView, NSString, UIView;
@protocol IESECGoodsFeedPopoverViewDelegate;

@interface IESECGoodsFeedPopoverView : UIView <IESECGoodsFeedPopoverItemDelegate> {
    BOOL _inTargetCenter;
    BOOL _showPoint;
    UIView *_target;
    unsigned long long _position;
    double _sharpOffset;
    double _shapeOffsetInCenter;
    id<IESECGoodsFeedPopoverViewDelegate> _delegate;
    IESECGoodsFeedPopoverItemContentView *_contentView;
    UIStackView *_stackView;
    NSArray *_configs;
    struct CGPoint { double x; double y; } _targetPoint;
    struct UIEdgeInsets { double top; double left; double bottom; double right; } _edgeinsets;
}

@property (retain, nonatomic) IESECGoodsFeedPopoverItemContentView *contentView;
@property (nonatomic) struct UIEdgeInsets { double top; double left; double bottom; double right; } edgeinsets;
@property (retain, nonatomic) UIStackView *stackView;
@property (retain, nonatomic) NSArray *configs;
@property (nonatomic) struct CGPoint { double x; double y; } targetPoint;
@property (retain, nonatomic) UIView *target;
@property (nonatomic) unsigned long long position;
@property (nonatomic) BOOL inTargetCenter;
@property (nonatomic) BOOL showPoint;
@property (nonatomic) double sharpOffset;
@property (nonatomic) double shapeOffsetInCenter;
@property (weak, nonatomic) id<IESECGoodsFeedPopoverViewDelegate> delegate;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

- (void)setConfigs:(id)arg0;
- (void)coverViewDidHit;
- (void)setSharpOffset:(double)arg0;
- (double)sharpOffset;
- (void)checkOverView;
- (void)addItemConfiguration:(id)arg0;
- (void)setShapeOffsetInCenter:(double)arg0;
- (void)setInTargetCenter:(BOOL)arg0;
- (void)tapWillFinish;
- (double)shapeOffsetInCenter;
- (BOOL)showPoint;
- (void)setShowPoint:(BOOL)arg0;
- (BOOL)inTargetCenter;
- (void)setEdgeinsets:(struct UIEdgeInsets { double x0; double x1; double x2; double x3; })arg0;
- (struct UIEdgeInsets { double x0; double x1; double x2; double x3; })edgeinsets;
- (void)addArrangedSubviewWithSeparator:(id)arg0;
- (id)init;
- (void)dismiss;
- (void).cxx_destruct;
- (unsigned long long)position;
- (void)setTarget:(id)arg0;
- (void)show;
- (void)setPosition:(unsigned long long)arg0;
- (id)target;
- (id)contentView;
- (id)delegate;
- (id)stackView;
- (void)setStackView:(id)arg0;
- (void)setDelegate:(id)arg0;
- (void)updateItems;
- (id)hitTest:(struct CGPoint { double x0; double x1; })arg0 withEvent:(id)arg1;
- (void)setContentView:(id)arg0;
- (struct CGPoint { double x0; double x1; })targetPoint;
- (void)setTargetPoint:(struct CGPoint { double x0; double x1; })arg0;
- (id)configs;

@end
