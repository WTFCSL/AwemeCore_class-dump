//
//     Generated by private class-dump
//

@class NSString;

@interface BDNovelComicExitShelfAlertParam : NSObject {
    BOOL _isDarkMode;
    NSString *_title;
    NSString *_subTitle;
    NSString *_imageUrl;
    NSString *_darkImageUrl;
    NSString *_cancelText;
    NSString *_confirmText;
    id /* block */ _emptyClickBlock;
    id /* block */ _cancelBlock;
    id /* block */ _closeBlock;
    id /* block */ _confirmBlock;
}

@property (copy, nonatomic) NSString *title;
@property (copy, nonatomic) NSString *subTitle;
@property (copy, nonatomic) NSString *imageUrl;
@property (copy, nonatomic) NSString *darkImageUrl;
@property (copy, nonatomic) NSString *cancelText;
@property (copy, nonatomic) NSString *confirmText;
@property (nonatomic) BOOL isDarkMode;
@property (copy, nonatomic) id /* block */ emptyClickBlock;
@property (copy, nonatomic) id /* block */ cancelBlock;
@property (copy, nonatomic) id /* block */ closeBlock;
@property (copy, nonatomic) id /* block */ confirmBlock;

- (id /* block */)closeBlock;
- (void)setCloseBlock:(id /* block */)arg0;
- (id /* block */)confirmBlock;
- (void)setConfirmBlock:(id /* block */)arg0;
- (void)setCancelText:(id)arg0;
- (id)cancelText;
- (void)setDarkImageUrl:(id)arg0;
- (id)darkImageUrl;
- (id /* block */)emptyClickBlock;
- (void)setEmptyClickBlock:(id /* block */)arg0;
- (id /* block */)cancelBlock;
- (void).cxx_destruct;
- (id)title;
- (void)setCancelBlock:(id /* block */)arg0;
- (void)setSubTitle:(id)arg0;
- (id)subTitle;
- (void)setTitle:(id)arg0;
- (id)imageUrl;
- (void)setImageUrl:(id)arg0;
- (BOOL)isDarkMode;
- (void)setIsDarkMode:(BOOL)arg0;
- (id)confirmText;
- (void)setConfirmText:(id)arg0;

@end
