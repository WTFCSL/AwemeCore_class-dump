//
//     Generated by private class-dump
//

@class NSLock, NSString;

@interface AWEIronManModuleService : HTSService <AWEIronManModuleService, AWEPerfResourceCleanStrategy> {
    NSLock *_cleanLock;
}

@property (retain, nonatomic) NSLock *cleanLock;
@property (nonatomic) long long ironManPublishStatus;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

+ (void)trackSearchAdIronManWithModel:(id)arg0 eventName:(id)arg1;
+ (void)handleProfileExtensionAreaActionWithType:(unsigned long long)arg0 userModel:(id)arg1 schema:(id)arg2;
+ (void)eventShowAndClickTrackerWithParams:(id)arg0;

- (void)onServiceInit;
- (Class)profileExtensionAreaCardControllerClass;
- (Class)newProfileExtensionAreaCardControllerClass;
- (void)trackIronMan:(id)arg0 event:(id)arg1 scene:(id)arg2 refer:(id)arg3 carrier:(id)arg4 groupId:(id)arg5 authorId:(id)arg6 extraInfo:(id)arg7;
- (long long)ironManPublishStatus;
- (void)setIronManPublishStatus:(long long)arg0;
- (void)showIronManODRToastWithContent:(id)arg0 onController:(id)arg1;
- (id)videoPlayTrackerParamsWithAweModel:(id)arg0;
- (void)setIronManTransitionDelegate:(id)arg0 toTransitionContainer:(id)arg1;
- (void)excuteAfterLaunch;
- (id)currentLibVersion;
- (void)registerJSSDKVersion;
- (void)clearDiskWithCompletion:(id /* block */)arg0 isAuto:(BOOL)arg1;
- (void)clearDiskExtremelyWithCompletion:(id /* block */)arg0;
- (long long)calculateCleanCacheSizeWithTriggerType:(long long)arg0 cleanLevel:(long long)arg1;
- (void)setupExtremelyPreloadDegrade:(BOOL)arg0;
- (void)trackIronMan:(id)arg0 event:(id)arg1 scene:(id)arg2 refer:(id)arg3 carrier:(id)arg4 groupId:(id)arg5 authorId:(id)arg6;
- (void)trackIronMan:(id)arg0 event:(id)arg1 refer:(id)arg2 carrier:(id)arg3 groupId:(id)arg4 authorId:(id)arg5;
- (void)trackIronManWithSchema:(id)arg0 event:(id)arg1 refer:(id)arg2 tagID:(id)arg3;
- (id)getPlatformChannel:(long long)arg0;
- (id)sceneQueryStringWithShareType:(long long)arg0;
- (void)visitAppWithAppSchema:(id)arg0;
- (id)ironManShareSchemeFromURL:(id)arg0;
- (long long)ironManTypeForURLString:(id)arg0;
- (id)gameModelIronManCenter;
- (id)platformChannelWithShareType:(id)arg0;
- (void)getShareInfoForShareChannel:(id)arg0 resultCallback:(id /* block */)arg1;
- (void)decodeShareTokenAuditToken:(id)arg0 completion:(id /* block */)arg1;
- (BOOL)isIronManURLString:(id)arg0;
- (id)baseNavigationController;
- (BOOL)isIronManUsing;
- (void)closeTimorViewController:(id)arg0;
- (BOOL)closeTopGameWithCompletion:(id /* block */)arg0;
- (void)clearAllGameWarmBootCache;
- (void)preloadTimorWithURL:(id)arg0 priority:(long long)arg1;
- (void)preloadIronManWithSchema:(id)arg0;
- (void)preloadIronManWithSchemaList:(id)arg0;
- (void)prepareTimorWithCaller:(id)arg0;
- (id)containerControllerWithURL:(id)arg0;
- (void)notifyTransitionWithDestinationController:(id)arg0 transitionState:(long long)arg1;
- (id)ironManTitle;
- (id)sceneAndBdpLogQueryStringWithPlatformType:(id)arg0 withSchema:(id)arg1;
- (void)pigeonToDetailTableViewControllerWithParam:(id)arg0 successBlock:(id /* block */)arg1;
- (id)ironManStorage;
- (Class)guestProfileExtensionAreaCardControllerClass;
- (void)updateIronManAnchorConfigIfNeed;
- (id)ironManPluginCommonInfoParams;
- (id)appidFromURL:(id)arg0;
- (id)allIronManTitle;
- (BOOL)isSpiderManSupportLandscape;
- (BOOL)isIronManOrSpiderManPresenting;
- (BOOL)currentGameUseNewArch;
- (void)setCurrentGameUseNewArch:(BOOL)arg0;
- (BOOL)gameUseNewArchABValueWithSchema:(id)arg0;
- (void)routeToIronManWithTransitionInfo:(id)arg0;
- (BOOL)isUsingAudioSession;
- (BOOL)isPlayingBgAudio;
- (BOOL)isNativeAppScreenRecording;
- (id)personPageIronManParams:(id)arg0;
- (id)createGameNoTestDebugPermissionVCWithAppId:(id)arg0;
- (BOOL)shouldShowMorePanelDebugItemWithAppId:(id)arg0;
- (id)easterEggDictWithAppId:(id)arg0;
- (void)easterEggRefreshBoundAppListWithAppId:(id)arg0;
- (void)changeDevInfoWithUid:(id)arg0 easterEggDict:(id)arg1;
- (BOOL)isDevModeOpenWithUid:(id)arg0;
- (void)updateLiveOpenPlatformAudienceVideoContainer:(id)arg0 status:(long long)arg1;
- (void)trackSpiderManMpShowForFeedAdWithContext:(id)arg0 aweModel:(id)arg1;
- (void)getShareResultModelForShareChannel:(id)arg0 withUniqueID:(id)arg1 callback:(id /* block */)arg2;
- (void)setUniqueIDForIromManShare:(id)arg0;
- (id)getUniqueIDForIronManShare;
- (id)cleanPaths;
- (void)cleanResourceWithStrategy:(unsigned long long)arg0;
- (double)cleanSizeWithStrategy:(unsigned long long)arg0;
- (id)cleanLock;
- (void)setCleanLock:(id)arg0;
- (void)handleTimorExit:(id)arg0;
- (id)getFeedAdTrackParamsWithContext:(id)arg0 aweModel:(id)arg1;
- (void).cxx_destruct;

@end
